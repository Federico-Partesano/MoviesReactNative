...
const ListOfCards: React.FC<Props> = ({route, navigation})  => {
  ...
  async function appendMovies(page: number) {
    ...
    const response: any = await axios.get(
      `https://api.themoviedb.org/3/${type}/${paramCategoryForFetch}?api_key=${keyApi}&language=it&region=it&page=${page}`
    );
    const arrayMovies = [...moviesList,...response.data.results];
    setMoviesList(arrayMovies);
    setPage(page + 1);
